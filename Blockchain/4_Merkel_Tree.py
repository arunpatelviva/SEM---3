...
class MerkelTree:
    """Stores the leaves and root hash of the tree."""
    def __init__(self,data_chunks):
        leaves=[]
        for chunk in data_chunks:
            node=MerkelNode(self.compute_hash(chunk))
            leaves.append(node)
        self.root=self.build_merkel_tree(leaves)
    def build_merkel_tree(self,leaves):
